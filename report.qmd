...
In our animated plot, we can see how even though the years included in the study are very recent, how every country is seemingly increasing their Mean Years of Schooling and their Life Expectancy. This is definitely most easily seen when the animation restarts, you can see the progress made by 2017 once it resets to 2000 very well. Over the years, the linear relationship remains, and in fact appears to grow stronger. This illustrates just how well related these two variables are.
...
We fit a simple linear regression model to predict a country's average life expectancy ($\hat{y}$) based on its average years of schooling ($mean\_edu$). The fitted regression equation is:

$$
\hat{y} = 54.941 + 0.289(mean\_edu)
$$
...
According to this graph we can assume our model is moderatly predictive. The mean R^2 value appears to be around .625. This means that on average our model explains around 62.5% of the variance in life expectancy. The R^2 values range pretty widely. It's from around .2 to above 2. This means that there is some high variance in the performance across folds. This is most likely due to some folds having a small sample size or there are some outliers in certain folds. There is a chance for some overfitting, but not guaranteed. One fold yields a very high R^2 value > 2. This could indicate that there is some overreacting to smaller datasets.
