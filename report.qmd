...
```{r setup}
# load packages
# loading tidyverse doesn't work for me
library(readxl) 
library(ggplot2)
library(dplyr)
library(tidyr)
...
https://stackoverflow.com/questions/7833173/lambda-like-functions-in-r
```{r}
# code for Q3
military_clean3 <- military_clean |>
  mutate(-c("Country") |>
           across(~ ifelse(.x == "..." | .x == "xxx", NA, .x)))
...
```{r}
# code for Q4
military_clean4 <- military_clean3 |>
  mutate(`1988.0`:`2023.0` |>
           across(as.numeric))
```
```{r}
#| eval: false
...
```{r}
# code for Q5
military_clean5 <- military_clean4 |>
  anti_join(cont_region, by = join_by(Country == Region))
```
...
```{r}
# code for Q6
military_clean6 <- military_clean5 |>
  filter(!if_all(-c("Country"), is.na))
military_clean5 |>
  filter(if_all(-c("Country"), is.na))
```
...
```{r}
# code for Q7
military_long <- military_clean6 |>
  pivot_longer(`1988.0`:`2023.0`,
               names_to = "year",
               values_to = "spending")
```
...
military_long8 |>
  filter(Country %in% eastern_europe) |>
  ggplot(mapping = aes(x = year, y = spending, color = Country)) +
  geom_line()
